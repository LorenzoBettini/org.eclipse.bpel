...
public final class BPELProxyURI
{
    ...
    public BPELProxyURI(EClass type, URI baseURI, QName qName)
    {
        ...
        this.qName = qName;
        if (qName == null || qName.getNamespaceURI() == null || qName.getNamespaceURI().equals(""))
        {
            //System.err.println("BPELProxyURI expects a QName with a namespace: " + qName);
        }
    }
    ...
    public BPELProxyURI(String uriFragment)
    {
        ...
        //
        // The format of the qname {namespace}localPart is dependent upon the implementation of QName.toString()
        // and the {namespace} part is optional so in theory this may not always parse correctly.
    	// The URIs we are dealing with may not have a namespace, so take this into account.
    ...
    private QName parseQName(String string) {
        try {
            ...
            System.err.println("Error parsing QName:" + string);
        }
        return null;
    }
    ...
    {
    	// The qname may or may not have a namespace. In the case that it doesn't,
    	// we still need the fragment URI string to contain {} so that our parsing
