...
public class ExpressionContentAssistProcessor
	...
	class XPathStack {
			...
			while (theIndex < theOffset) {
				...
				// literal
					...
					if ((theTopOfStackExprType == ExpressionType.EXPRTYPE_BOOLEAN_OPERATOR) ||
						...
						if (currChar == '-') {
							theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_UNARY_OPERATOR, "-"));
							theTopOfStackExprType = ExpressionType.EXPRTYPE_UNARY_OPERATOR;
							theIndex++;
							if (parseNumberExpression()) {
								...
								theCallStack.pop();
								// push "numeric" value
								theCallStack.push(new ExpressionType(ExpressionType.CLASS_NUMERIC, ""));
						...
						if ((theIndex+1) < theOffset) {
							...
							int tempType = 0;
							if ((tempOper.compareTo("+") == 0) || (tempOper.compareTo("-") == 0) || 
									(tempOper.compareTo("*") == 0))
					...
					if (parseBooleanExpression()) {
						...
						theCallStack.push(new ExpressionType(ExpressionType.CLASS_EXPRESSION, ""));
					...
					try {
						...
						if (tempExpr != null) {
							if (tempExpr.theType == ExpressionType.EXPRTYPE_NEW_EXPRESSION) {
								switch (currChar) {
								case ')':
									if (tempExpr.theGrammar.compareTo("(") == 0)
										return true;
									else {
										theStatus = -1;
										return false;
		...
		private boolean parseFunction() throws BadLocationException {			
			...
			while (theIndex < theOffset) {
				nextChar = theDocument.getChar(theIndex);
				if (nextChar == '(')
					theCallStack.push(new ExpressionType(ExpressionType.EXPRTYPE_FUNCTION_ARGUMENTS, "("));
				else if (nextChar == ')') {
					try {
						ExpressionType tempExpr = (ExpressionType)theCallStack.peek();
	...
	private boolean theToggle = true;
	...
	private ICompletionProposal[] generateProposals(ITextViewer viewer, int offset) {
		...
		if (propType.isVariableAndFunction()) {
			theToggle = !theToggle;	
			if (theToggle) {
				theContentAssistant.setStatusMessage("Press 'Ctrl+Space' to show Function Proposals");
				return generateVariableProposals(propType.theBeginsWith, offset);			}
			else {
				theContentAssistant.setStatusMessage("Press 'Ctrl+Space' to show Variable Proposals");				
				return generateFunctionProposals(viewer,propType.theBeginsWith, offset);
			}		
		}
		else if (propType.isVariable()) {
			theContentAssistant.setStatusMessage("");
			return generateVariableProposals(propType.theBeginsWith, offset);
		}
			...
			theContentAssistant.setStatusMessage("");			
			return generateOperatorProposals("", offset);
		}
			...
			if (expr != null) {		
				...
				case ExpressionType.EXPRTYPE_VARIABLE:
					if (callStack.theStatus == 1)
						...
						return (new ProposalType(ProposalType.PROPTYPE_OPERATOR, "")); 
				...
				case ExpressionType.EXPRTYPE_NEW_EXPRESSION:
					return (new ProposalType(ProposalType.PROPTYPE_FUNCTION | ProposalType.PROPTYPE_VARIABLE, ""));
	...
	private ICompletionProposal[] generateOperatorProposals(String context, int offset) {
		final String[] OPERATOR_LIST = {"and", "or", "=", "!=", "<", ">", "<=", ">=", "+", "-", "*", "div", "mod"};
	...
	private ICompletionProposal[] generateVariableProposals(String context, int offset) {
		...
		if (seekChildren) {
			...
			while (index < context.length()) {
				t = context.charAt(index);
				if ((t == '.') || (t == '/')) {
					levelName = context.substring(token+1, index);
					// check for namespace
					...
					// traverse down
						...
						if (context.charAt(token) == '.') {
							Message msg = currVar.getMessageType();
							if (msg != null) {
						...
						// search for child objects
							...
							if (currXsdType instanceof XSDComplexTypeDefinition) {
								...
								while (elemIter.hasNext()) {
									XSDElementDeclaration elem = (XSDElementDeclaration)elemIter.next();
									if (levelName.compareTo(elem.getName()) == 0) {
			...
			if (index == context.length()) {
				...
				if (index == token)
					...
					beginsWith = context.substring(token+1);
				
				if ((context.charAt(token) == '/') || (context.charAt(token) == '@')){
					if (currXsdType instanceof XSDComplexTypeDefinition) {
						XSDComplexTypeDefinition xsdcomplex = (XSDComplexTypeDefinition)currXsdType;
						Iterator eaIter;
						if (context.charAt(token) == '/')
						...
						while (eaIter.hasNext()) {
							...
							if (tempEA instanceof XSDAttributeDeclaration) {
								...
								tempReplName = "@" + attr.getName();
								tempDispName = attr.getName();
								img = attrImg;
							}
							else if (tempEA instanceof XSDElementDeclaration) {
								XSDElementDeclaration elem = (XSDElementDeclaration)tempEA;
							...
							if (tempReplName != null) {
								if ((beginsWith.length() == 0) || (tempDispName.startsWith(beginsWith))) {
									...
									prop = new CompletionProposal(tempReplName, replOffset, replLen,
											tempReplName.length(), img, tempDispName + "   " ,
											null, null);
									results.add(prop);
								}
							}
				...
				// search for parts
					...
					Message msg = currVar.getMessageType();
					Map list = msg.getParts();
					Collection coll = list.values();
					Part item = (Part)coll.iterator().next();
	...
	private boolean isReservedOperatorCharacter(char c) {
		final String RESERVED_OPERATOR_CHARS = "+-*";
	...
	private boolean isClosingExpressionCharacter(char c) {
		final String RESERVED_CLOSING_EXPR_CHARS = ")]";
