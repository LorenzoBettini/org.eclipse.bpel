...
public class JoinFailureSection extends BPELPropertySection {
	...
	protected MultiObjectAdapter[] createAdapters() {
		...
		new MultiObjectAdapter() {
			...
			public void notify(Notification n) {
				if (!isCreated) {
					// HACK: we've been removed already, but the removal
					// occurred
					// during another notification, which is still sent to us.
					// TODO: where else might this late notification occur?
					// TODO: is there a generic workaround which isn't too
					// costly?
					return;
				}
				if (ModelHelper.isSuppressJoinFailureAffected(n.getNotifier(),
						n)) {
					updateSuppressJoinFailureWidgets();
				}
				if (n.getNotifier().equals(getInput().eContainer())
						&& getInput().eContainmentFeature().equals(
								n.getFeature())) {
					// The model object was reparented, so the meaning of our
					// default
					// value may have changed. Reset our adapters and update the
					// widget.
					removeAllAdapters();
					addAllAdapters();
					updateSuppressJoinFailureWidgets();
	...
	protected void addAllAdapters() {
		...
		for (EObject p = getInput().eContainer(); p != null; p = p.eContainer()) {
			if (ModelHelper.supportsJoinFailure(p))
				adapters[0].addToObject(p);
		}
	}
	...
	protected void createSuppressJoinFailureWidgets (Composite composite) {
		...
		data.right = new FlatFormAttachment(yesRadio,
				-IDetailsAreaConstants.HSPACE);
		...
		SelectionListener selectionListener = new SelectionListener() {
			public void widgetSelected(SelectionEvent e) {
				...
				getCommandFramework().execute(
						wrapInShowContextCommand(new SetCommand(getInput(),
								value, getFeature())));
			}
	...
	protected void updateSuppressJoinFailureWidgets() {
		...
		yesRadio.setEnabled(enable);
		noRadio.setEnabled(enable);
		sameAsParentRadio.setEnabled(enable);
		
		if (enable == false) {
			return ;
		}					
		
		// TODO: need boolean flag here?
		// joinFailureChangeHelper.startNonUserChange();
		try {
			String newDefault = inheritSuppressed() ? Messages.JoinFailureDetails_Use_Same_Value_as_Parent__Yes__5
			...
			int index = suppressJoinFailureIndex(ModelHelper
					.getSuppressJoinFailure(getInput()));
			if ((index == 2) && (getInput() instanceof Process))
				index = 1;
			...
			// TODO: need boolean flag here?
			// joinFailureChangeHelper.finishNonUserChange();
		}
	}
	...
	protected void doChildLayout() {
		sameAsParentRadio.setVisible(getInput() instanceof Process == false);
	}

	EStructuralFeature getFeature() {
		if (getInput() instanceof Process) {
			return BPELPackage.eINSTANCE.getProcess_SuppressJoinFailure();
		}
		return BPELPackage.eINSTANCE.getActivity_SuppressJoinFailure();
	}
